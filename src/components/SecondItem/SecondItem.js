...
const SecondItem = (props) => {
    ...
    async function right_item (id) {
        ...
        const boats = await SecondService.getList(props.firstId)

        const common_boats = boats.filter((elem)=> elem.boat_type == boats[props.firstId-1].boat_type)

        const res = common_boats.find((elem) => { return elem.id > props.firstId})
        //common_boats.find()
        await SecondService.patch(res.id, id)
    ...
    return (
        <div className={classes.task}>
            ...
            <Modal visible={visible} setVisible={setVisible}>
                <FormEdit id={props.body.id} firstId={props.firstId} itemList={props.itemList} setItems={props.setItems}  body={props.body} visible={visible} setModalVisible={setVisible}/>
            </Modal>
        </div>
    );
};
